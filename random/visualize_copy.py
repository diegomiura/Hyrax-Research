            ...
            if i < len(sampled_ids):
                try:
                    ...
                    if cutout_path.suffix.lower() == ".fits":
                        ...
                        # # Use regex to swap the band token (G/R/I) irrespective of '_' or '-' separators
                        # import re

                        # cutout_path_str = str(cutout_path)

                        # def swap_band(path_str: str, band: str) -> str:
                        #     # Pattern 1: single-letter band between separators, e.g., _G_ or -r-
                        #     pat_mid = re.compile(r'([_\-])([gri])([_\-])', flags=re.IGNORECASE)
                        #     if pat_mid.search(path_str):
                        #         return pat_mid.sub(lambda m: f"{m.group(1)}{band}{m.group(3)}", path_str, count=1)

                        #     # Pattern 2: band at the end before extension, preceded by a separator, e.g., _I.fits or -g.fits
                        #     pat_end = re.compile(r'([_\-])([gri])(\.fits?)$', flags=re.IGNORECASE)
                        #     if pat_end.search(path_str):
                        #         return pat_end.sub(lambda m: f"{m.group(1)}{band}{m.group(3)}", path_str, count=1)

                        #     # If neither pattern matches, leave unchanged and warn
                        #     logger.warning(f"Could not identify band token in filename: {path_str}")
                        #     return path_str

                        # # Build paths for each desired band
                        # g_path_str = swap_band(cutout_path_str, "G")
                        # r_path_str = swap_band(cutout_path_str, "R")
                        # i_path_str = swap_band(cutout_path_str, "I")

                        # cutout_path_g = Path(g_path_str)
                        # cutout_path_r = Path(r_path_str)
                        # cutout_path_i = Path(i_path_str)

                        # print(cutout_path_g)
                        # print(cutout_path_r)
                        # print(cutout_path_i)

                        # # Re-attach base_dir if we were given a relative path
                        # # if not cutout_path_g.is_absolute():
                        # #     cutout_path_g = base_dir / cutout_path_g
                        # if not cutout_path_r.is_absolute():
                        #     cutout_path_r = base_dir / cutout_path_r
                        # if not cutout_path_i.is_absolute():
                        #     cutout_path_i = base_dir / cutout_path_i

                        # arr_g = fits.getdata(base_dir / cutout_path)
                        # arr_r = fits.getdata(cutout_path_r)
                        # arr_i = fits.getdata(cutout_path_i)

                        # arr = np.stack([arr_g, arr_r, arr_i], axis=-1)
